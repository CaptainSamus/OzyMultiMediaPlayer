...
const Sequence = {
  ...
  cacheKey(seq, dir, exposure, colour, firstMtime) {
    const s = [Sequence.PIPELINE, dir, seq.name, seq.sep, seq.pad, String(seq.ext).toLowerCase(), seq.start, seq.end, Math.round(firstMtime || 0), Sequence.isExr(seq) ? `${Number(exposure) || 0}|${colour}` : ''].join('|');
