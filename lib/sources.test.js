...
test('kindOf classifies by extension', () => {
  ...
  assert.equal(Sources.kindOf('a.0001.EXR'), 'frame');
  assert.equal(Sources.kindOf('a.dpx'), 'frame');
});
test('filterMedia with images also keeps exr / tif / dpx frames', () => {
  assert.deepEqual(Sources.filterMedia(['a.exr', 'b.tif', 'c.mp4', 'd.dpx'], { images: true }), ['a.exr', 'b.tif', 'c.mp4', 'd.dpx']);
  assert.deepEqual(Sources.filterMedia(['a.exr', 'c.mp4']), ['c.mp4']);
});
